class Soundex(object):
    ...
    def encode(self, word):
        return self.__zero_pad(self.__upper_front(self.__head(word))) + \
               self.__tail(self.__encoded_digits(word))

    def encoded_digit(self, letter):
        find = self.encodingsMap[self.__lower(letter)]
        if find is not None:
            return find
        else:
    ...
    def __encoded_digits(self, word):
        encoding = self.empty_string
        self.__encode_head(encoding, word)
        self.__encode_tail(encoding, word)
        return encoding

    def __encode_head(self, encoding, word):
        if len(word) is 0:
            return
        encoding += self.encoded_digit(word[:1])

    def __encode_tail(self, encoding, word):
        word_length = len(word)
        for letter_index in range(1, word_length):
            if self.__is_complete(encoding) is False:
                self.__encode_letter(encoding, word[letter_index], word[letter_index-1])
    ...
    def __zero_pad(self, word):
        zeros_needed = len(word) - self.max_code_length
